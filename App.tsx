...
// DRM — Google's Widevine test asset (Android) and a public multi-DRM test
// vector shared across platforms.
...
const SHARED_MPD = 'https://media.axprod.net/TestVectors/Cmaf/protected_1080p_h264_cbcs/manifest.mpd';
const SHARED_HLS = 'https://media.axprod.net/TestVectors/Cmaf/protected_1080p_h264_cbcs/manifest.m3u8';
const SHARED_WV_LICENSE = 'https://drm-widevine-licensing.axtest.net/AcquireLicense';
const SHARED_FP_CERT = 'https://vtb.axinom.com/FPScert/fairplay.cer';
const SHARED_FP_LICENSE = 'https://drm-fairplay-licensing.axtest.net/AcquireLicense';
const SHARED_TOKEN =
...
function DrmDemo() {
  ...
  const tokenSource: OGMediaItem = {
    url: Platform.OS === 'android' ? SHARED_MPD : SHARED_HLS,
    title: 'Multi-DRM demo (encrypted)',
    drm: {
      widevine: { licenseUrl: SHARED_WV_LICENSE },
      fairplay: { certificateUrl: SHARED_FP_CERT, licenseUrl: SHARED_FP_LICENSE },
      tokenHeaderName: 'X-AxDRM-Message',
      tokenProvider: async ({ renewal }) => {
        log.add(`tokenProvider called (renewal=${renewal})`);
        return { 'X-AxDRM-Message': SHARED_TOKEN };
      },
    },
  };
  const iosOnlyToken = Platform.OS !== 'android';
  // The open Widevine asset has one audio track and no text tracks; the
  // shared vector has three of each. Buttons that would open an empty menu
  ...
  return (
    ...
    >
      ...
      <Text style={s.caption}>
        ...
        {Platform.OS === 'android'
          ? ' The open Widevine asset has one audio track and no subtitles, so its subtitle and audio buttons are hidden; the shared stream has three of each and shows both.'
          : ''}
      </Text>
      <EventLog lines={log.lines} />
    </Screen>
...
function DownloadsDemo() {
  ...
  // The same streams the DRM demo uses; the entitlement message
  ...
  const maybeLoadCompleted = useCallback(
    (ds: OGDownload[]) => {
      ...
      setPlaySource(item(done.url === SHARED_MPD));
      setGen((g) => g + 1);
    },
    [addLog, item]
  );
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      OGDownloads.list()
        .then((ds) =>
          ds.forEach((d) => OGDownloads.remove(d.url, d.url === SHARED_MPD ? item(true) : undefined))
        )
        .catch(() => {});
    };
  }, [addLog, item, refresh]);
